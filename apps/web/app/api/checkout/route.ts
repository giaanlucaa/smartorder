...
export async function POST(req: NextRequest) {
  ...
  try {
    const { venueId, tableToken, cartData, sessionId, tipAmount = 0 } = await req.json();
    ...
    const taxAmount = subtotal * 0.081; // 8.1% VAT
    const tipAmountNum = parseFloat(tipAmount.toString()) || 0;
    const finalTotal = subtotal + taxAmount + tipAmountNum;
