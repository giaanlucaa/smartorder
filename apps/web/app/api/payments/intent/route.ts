...
export async function POST(req: NextRequest) {
  ...
  const order = await prisma.order.findUnique({ where: { id: orderId }, include: { venue: true } });
  ...
  if (paymentMethod === 'direct' || paymentMethod === 'cash') {
    ...
    return NextResponse.json({
      providerRef: `${provider}_${Date.now()}`,
      redirectUrl: `${baseUrl}/success?orderId=${order.id}&payment=${provider}`
    });
  }
  ...
  if (!hasPaymentProvider) {
    ...
    return NextResponse.json({
      providerRef: "test_" + Date.now(),
      redirectUrl: `${baseUrl}/success?orderId=${order.id}&payment=${paymentMethod || 'test'}`
    });
  }
