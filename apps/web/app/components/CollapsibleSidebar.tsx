...
export default function CollapsibleSidebar({ venue, collapsed = false }: CollapsibleSidebarProps) {
  ...
  return (
    <div className={`${collapsed ? 'w-16' : 'w-96'} bg-gradient-to-b from-gray-900 to-gray-800 border-r border-gray-700 h-full overflow-y-auto shadow-lg transition-all duration-300`}>
      {/* Header */}
      <div className={`${collapsed ? 'p-2' : 'p-6'} border-b border-gray-700 bg-gradient-to-r from-gray-800 to-gray-900`}>
        <div className={`flex items-center ${collapsed ? 'justify-center' : 'space-x-3'} ${collapsed ? '' : 'mb-4'}`}>
          ...
          return (
            <div key={index} className="mb-4">
              <button
                ...
                className={`w-full flex items-start ${collapsed ? 'justify-center' : 'space-x-3'} mb-2 p-2 rounded-lg hover:bg-gray-700/50 transition-colors`}
              >
